...
from environment import servers, cloud, B_aver, B_c, B_e, B_u, request_list
class Task:
    ...
    def __get_est(self, t, p):
        ...
        for pre in self.tasks:
            if self.graph[pre.id][t.id] != -1:  # if pre also done on p, no communication cost
                ...
                if pre.processor_id in range(5) and p.id in range(5):   # 前后两个都在边缘服务器上
    ...
    def __allotProcessor(self):
        for t in self.tasks:
            if t == self.tasks[0]:   # the one with highest rank
                # p = np.random.choice(range(5)) # 随机从某个边缘服务器发出请求
                p = request_list[self.k]
                w = t.comp_cost[p]
                t.processor_id = p
                t.duration['start'] = 0
                t.duration['end'] = w
...
if __name__ == "__main__":
    ...
    ap = ArgumentParser()
    DAGS = np.load('dag_info.npy', allow_pickle=True)
