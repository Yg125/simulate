...
class OnDoc:
    ...
    def LT(self, k):
        for t in self.dags[k].tasks:
            t.lt = self.dags[k].r + (self.dags[k].deadline - self.dags[k].r) * (self.dags[k].tasks[0].rank - t.rank + t.avg_comp) / self.dags[k].tasks[0].rank
    
    def arrive(self, k):
        self.computeRank(self.dags[k].tasks[0], k)
        self.LT(k)
        result = self.dags[k].tasks
        result.sort(key=lambda x: x.lt)
    ...
    def str(self):
        ...
        print_str += "OnDoc2:SR = {}%\n".format(SR)
        print_str += "OnDoc2:Average Makespan = {}\n".format(average_makespan)
        return print_str
